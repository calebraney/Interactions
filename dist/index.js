(() => {
  ...
  var tabsAnimation = function() {
    ...
    wraps.forEach((wrap, componentIndex) => {
      ...
      let ease = attr("power1.out", wrap.getAttribute(EASE));
      let previousButton = wrap.querySelector(`${PREV_BTN} button`), nextButton = wrap.querySelector(`${NEXT_BTN} button`), toggleWrap = wrap.querySelector(PLAY_BTN), toggleButton = wrap.querySelector(`${PLAY_BTN} button`), buttonList = wrap.querySelector(".tab_button_list"), panelList = wrap.querySelector(".tab_content_list"), animating = false, autoplayTl;
      ...
      const makeActive = (index, focus = false, animate = true, pause = true) => {
        ...
        buttonItems.forEach((btn, i2) => {
          btn.classList.toggle(ACTIVE_CLASS, i2 === index);
          btn.setAttribute("aria-selected", i2 === index ? "true" : "false");
          btn.setAttribute("tabindex", i2 === index ? "0" : "-1");
        });
        panelItems.forEach((panel, i2) => panel.classList.toggle(ACTIVE_CLASS, i2 === index));
      ...
      buttonItems.forEach((btn, index) => {
        ...
        if (new URLSearchParams(location.search).get("tab-id") === tabId + "-" + itemId)
          makeActive(index), autoplay = 0, wrap.scrollIntoView({ behavior: "smooth", block: "start" }), history.replaceState(
            {},
            "",
        ...
        btn.addEventListener("keydown", (e2) => {
          if (["ArrowRight", "ArrowDown"].includes(e2.key)) updateIndex(1, true);
          else if (["ArrowLeft", "ArrowUp"].includes(e2.key)) updateIndex(-1, true);
        });
      });
      if (autoplay !== 0 && typeof gsap !== "undefined") {
        let updateAuto = function() {
          ...
          updateAuto();
        }, handleMotionChange = function(e2) {
        ...
        wrap.addEventListener("focusout", (e2) => {
          if (!e2.relatedTarget || !wrap.contains(e2.relatedTarget)) {
            hasFocusInside = false;
            updateAuto();
          }
        });
