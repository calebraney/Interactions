(() => {
  ...
  var imageSwitch = function(gsapContext) {
    ...
    wraps.forEach((wrap) => {
      ...
      const activateItem = function(index, activate2 = true) {
  ...
  var lightbox = function(pagePlayers, pagePlayerComponents, lenis) {
    ...
    const activateLightboxes = function(listElement) {
      ...
      checkContainer(listElement, breakpoint, (match) => {
        isDisabled = match;
      });
    };
    const body = document.querySelector("body");
  ...
  var magnetic = function() {
    ...
    wraps.forEach((wrap) => {
      ...
      const breakpoint = attr("none", wrap.getAttribute(`data-ix-${ANIMATION_ID}-breakpoint`));
      checkContainer(wrap, breakpoint, (match) => {
        isDisabled = match;
      });
    });
  };
  ...
  var modal = function(lenis) {
    ...
    modals.forEach((modal2, index) => {
      ...
      const breakpoint = attr("none", modal2.getAttribute(`data-ix-${ANIMATION_ID}-breakpoint`));
      checkContainer(modal2, breakpoint, (match) => {
        isDisabled = match;
      });
    });
  ...
  var scrolling = function() {
    ...
    wraps.forEach((wrap) => {
      ...
      const animation = function(match) {
        if (match) return;
        let tl;
        const buildTimeline = function(altMatch) {
          if (tl) tl.kill();
          ...
          if (altMatch && wrap.getAttribute(BREAKPOINT_START)) {
            tlSettings.start = attr(tlSettings.start, wrap.getAttribute(BREAKPOINT_START));
          }
          if (altMatch && wrap.getAttribute(BREAKPOINT_END)) {
        ...
        const altBreakpoint = attr("none", wrap.getAttribute(`data-ix-${ANIMATION_ID}-alt-breakpoint`));
        checkContainer(items[0], altBreakpoint, buildTimeline);
      };
      const breakpoint = attr("none", wrap.getAttribute(`data-ix-${ANIMATION_ID}-breakpoint`));
      checkContainer(items[0], breakpoint, animation);
    });
      ...
      const swiper = new Swiper(swiperElement, {
        ...
        pagination: {
          ...
          el: component.querySelector(
            config.paginationType === "fraction" ? PAGINATION_FRACTION : config.paginationType === "progressbar" ? PAGINATION_PROGRESSBAR : PAGINATION_BULLETS
          ),
          ...
          ...config.paginationType === "fraction" && {
            renderFraction: function(currentClass, totalClass) {
              return '<span class="' + currentClass + ' slider_fraction_current"></span>' + config.fractionSeparator + '<span class="' + totalClass + ' slider_fraction_total"></span>';
            },
            ...config.fractionPad && {
  ...
  // src/interactions/number-input.js
  init_live_reload();
  var HIDE_ARROWS_CLASS = "ix-number-input-no-arrows";
  var injectHideArrowsStyle = () => {
    ...
    wraps.forEach((wrap) => {
      ...
      const animation = function(match) {
        ...
        const config = getAttrConfig(wrap, ANIMATION_ID, {
          ...
          // in em
          transitionDelay: 0,
          // sentinel: -1 defaults to config.duration (sequential)
          direction: "down",
          // 'up' or 'down'
          y: 1
